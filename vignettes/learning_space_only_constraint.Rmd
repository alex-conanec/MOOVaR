...
\begin{align}
    &\left\{
        \begin{array}{ll}
            ...
            \frac{1}{|E|} \sum\limits_{i \in E} K_H({\bf x}^{(1)} - {\bf X}^{(1)}_i) \geq \Gamma(\alpha/N) ~&\text{sinon si}~ |E| \geq d_1 + 1 \\
            0 ~&\text{sinon,}
        \end{array}
    \right. \\
    \text{avec}~& E = \left\{
    ...
    & N = \left\{
        \begin{array}{ll}
            \prod\limits_{j=1}^{d_2} m_j, ~&\text{si}~ d_2 > 0 \\
            1 ~&\text{sinon}
        \end{array}
    \right.\\
...
où $|M|$ dénote la cardinalité de l'ensemble $M$, $K_H$ est un noyau multi-gaussien pondéré par une largeur de fenêtre mobile $H$ de dimension $d_1 \times d_1$ et $\alpha$ est le risque d'affirmer que ${\bf x} \not\in \mathcal{X}$ alors que cela est vrai.   
